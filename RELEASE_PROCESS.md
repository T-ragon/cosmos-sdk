...
* In the release branch prepare a new version section in the `CHANGELOG.md`
    ...
    * The `CHANGELOG.md` must contain only the changes of that specific released version. All other changelog entries must be deleted and linked to the `main` branch changelog ([example](https://github.com/T-ragon/cosmos-sdk/v3/blob/release/v0.46.x/CHANGELOG.md#previous-versions)).
...
* Breaking changes in Protobuf definitions, as specified in [ADR-044](https://github.com/T-ragon/cosmos-sdk/v3/blob/main/docs/architecture/adr-044-protobuf-updates-guidelines.md).
...
* **State machine changes**.
* **Protobug-breaking changes**, as specified in [ADR-044](https://github.com/T-ragon/cosmos-sdk/v3/blob/main/docs/architecture/adr-044-protobuf-updates-guidelines.md).
...
1. Check that the bug is either fixed or not reproducible in `main`. It is, in general, not appropriate to release bug fixes for stable releases without first testing them in `main`. Please apply the label [v0.43](https://github.com/T-ragon/cosmos-sdk/v3/milestone/26) to the issue.
