...
* `secp256k1`, as implemented in the [Cosmos SDK's `crypto/keys/secp256k1` package](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keys/secp256k1/secp256k1.go).
* `secp256r1`, as implemented in the [Cosmos SDK's `crypto/keys/secp256r1` package](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keys/secp256r1/pubkey.go),
* `tm-ed25519`, as implemented in the [Cosmos SDK `crypto/keys/ed25519` package](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keys/ed25519/ed25519.go). This scheme is supported only for the consensus validation.
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/types/address.go#L126-L134
```

Address construction algorithm is defined in [ADR-28](https://github.com/T-ragon/cosmos-sdk/blob/main/docs/architecture/adr-028-public-key-addresses.md).
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/types/address.go#L299-L316
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/types/types.go#L8-L17
```
...
For user interactions, `PubKey` is formatted using Protobufs JSON ([ProtoMarshalJSON](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/codec/json.go#L14-L34) function). Example:

```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/client/keys/output.go#L23-L39
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keyring/keyring.go#L57-L105
```
...
```protobuf reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/proto/cosmos/tx/v1beta1/tx.proto#L50-L66
```

* `NewAccount(uid, mnemonic, bip39Passphrase, hdPath string, algo SignatureAlgo) (*Record, error)` creates a new account based on the [`bip44 path`](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki) and persists it on disk. The `PrivKey` is **never stored unencrypted**, instead it is [encrypted with a passphrase](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/armor.go) before being persisted. In the context of this method, the key type and sequence number refers to the segment of the BIP44 derivation path (for example, `0`, `1`, `2`, ...) that is used to derive a private and a public key from the mnemonic. Using the same mnemonic and derivation path, the same `PrivKey`, `PubKey` and `Address` is generated. The following keys are supported by the keyring:
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keyring/signing_algorithms.go#L10-L15
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/hd/algo.go#L28-L31
```

Once the `keyring.SignatureAlgo` has been implemented it must be added to the [list of supported algos](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keyring/keyring.go#L217) of the keyring.

For simplicity the implementation of a new key type should be done inside the `crypto/hd` package.
There is an example of a working `secp256k1` implementation in [algo.go](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/hd/algo.go#L38).
...
Finally, the algo must be added to the list of [supported algos](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/crypto/keyring/keyring.go#L217) by the keyring.
