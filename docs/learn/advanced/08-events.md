...
```go reference
https://github.com/T-ragon/cosmos-sdk/v3/blob/v0.50.0-alpha.0/types/events.go#L19-L26
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/v3/blob/v0.50.0-alpha.0/types/events.go#L53-L62
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/v3/blob/v0.50.0-alpha.0/x/group/keeper/msg_server.go#L95-L97
```
...
The `type` and `attribute` value of the `query` allow you to filter the specific Event you are looking for. For example, a `Mint` transaction triggers an Event of type `EventMint` and has an `Id` and an `Owner` as `attributes` (as defined in the [`events.proto` file of the `NFT` module](https://github.com/T-ragon/cosmos-sdk/v3/blob/v0.50.0-alpha.0/proto/cosmos/nft/v1beta1/event.proto#L21-L31)).
...
The same way can be used to subscribe to [legacy events](https://github.com/T-ragon/cosmos-sdk/v3/blob/v0.50.0-alpha.0/x/bank/types/events.go).
