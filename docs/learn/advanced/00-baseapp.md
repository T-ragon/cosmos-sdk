...
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/baseapp.go#L58-L182
...
* [`TxDecoder`](https://pkg.go.dev/github.com/T-ragon/cosmos-sdk/types#TxDecoder): It is used to decode
...
possibility to provide additional [`options`](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/options.go)
...
The [default `msgServiceRouter` included in `BaseApp`](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/msg_service_router.go) is stateless. However, some applications may want to make use of more stateful routing mechanisms such as allowing governance to disable certain routes or point them to new modules for upgrade purposes. For this reason, the `sdk.Context` is also passed into each [route handler inside `msgServiceRouter`](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/msg_service_router.go#L31-L32). For a stateless router that doesn't want to make use of this, you can just ignore the `ctx`.
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/auth/ante/basic.go#L102
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/baseapp.go#L663-L680
```
...
For more detailed examples, see the [`auth` module's `AnteHandler`](https://github.com/T-ragon/cosmos-sdk/tree/main/x/auth) which is widely used for these purposes.
...
`BaseApp` holds an `anteHandler` as parameter that is initialized in the [application's constructor](../beginner/00-app-anatomy.md#application-constructor). The most widely used `anteHandler` is the [`auth` module](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/auth/ante/ante.go).
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/auth/posthandler/post.go#L1-L15
```
...
```go reference 
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/abci.go#L623
```
...
* Initialize [`finalizeBlockState`](#state-updates) with the latest header using the `req abci.RequestFinalizeBlock` passed as parameter via the `setState` function.
  ...
  ```go reference
  https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/baseapp.go#L682-L706
  ```
...
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/baseapp.go#LL708-L743
...
```go reference 
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/baseapp.go#L747-L769
```
...
``` go reference 
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/abci_utils.go#L274-L281
```
...
```go reference
https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/baseapp/abci_utils.go#L282-L288
```
