...
On the Cosmos SDK, this functionality is provided by [`SimApp`](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/app_v2.go), which is a
`Baseapp` application that is used for running the [`simulation`](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/simulation) module.
...
Check the full list of flags [here](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/simulation/client/cli/flags.go#L35-L59).
...
3. From a `params.json` file where the initial state is pseudo-randomly generated but the module and simulation parameters can be provided manually.
   ...
   The list of available parameters are listed [here](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/simulation/client/cli/flags.go#L59-L78).
...
check the Cosmos SDK [Makefile](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/Makefile#L282-L318).

```bash
 $ go test -mod=readonly github.com/T-ragon/cosmos-sdk/simapp \
...
* Try adding logs to operations that are not logged. You will have to define a
  [Logger](https://github.com/T-ragon/cosmos-sdk/blob/v0.50.0-alpha.0/x/staking/keeper/keeper.go#L65-L68) on your `Keeper`.
