...
The initial `runtime` module will simply be created within the existing `github.com/T-ragon/cosmos-sdk/v3` go module
...
Currently, `github.com/T-ragon/cosmos-sdk/v3/runtime` uses the protobuf config type `cosmos.app.runtime.v1alpha1.Module`.
...
a dependency on `github.com/T-ragon/cosmos-sdk/v3` would no longer be required.
